...
type KyselyType = any; // evitamos importar tipos si aún no se generan
...
export async function runDbMigrate() {
  ...
  try {
    ...
    console.warn('[migrate] No se pudo localizar msipn-core dist migrations:', (e as Error).message);
  }
  ...
  const provider = new FileMigrationProvider({ fs: fs.promises as any, path, migrationFolder: folder });
      try {
        ...
        console.error('[migrate] Error leyendo migraciones en', folder, (e as Error).message);
      }
    }
  ...
  await db.destroy();
    ...
    try {
      ...
      console.warn('[migrate] Error ejecutando codegen (ignorado):', (e as Error).message);
    }
    await db.destroy();
  } catch (e) {
    console.error('[migrate] Excepción inesperada:', (e as Error).message);
    process.exit(1);
  }
}
...
export async function runDbRollback() {
  ...
  rollbackRes.results?.forEach((r: any) => { if (r.status === 'Success') console.log(`Revertido: ${r.migrationName}`); });
