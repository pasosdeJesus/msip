...
export async function runDbRollback() {
  const kyselyBin = path.join(process.cwd(), 'node_modules', '.bin', 'kysely');
  const codegenBin = path.join(
    process.cwd(), 'node_modules', '.bin', 'kysely-codegen'
  );

  if (!fs.existsSync(kyselyBin)) {
    problem(
      '[db:rollback] `kysely` binary not found. ' +
  ...
  if (!fs.existsSync(codegenBin)) {
    problem(
      '[db:rollback] `kysely-codegen` binary not found. ' +
        'Is it installed in your project?'
    );
    process.exit(1);
  }
  try {
    ...
    const rollback = spawnSync(
      kyselyBin, ['migrate:down'], { stdio: 'inherit' }
    );

    if (rollback.status !== 0) {
      problem(
        `[db:rollback] Kysely migrate:down failed ` +
    ...
    info(
      '[db:rollback] Executing: ./node_modules/.bin/kysely-codegen --out-file ./db/db.d.ts'
    );
    const codegen = spawnSync(
      codegenBin, ['--out-file', './db/db.d.ts'], { stdio: 'inherit' }
    );

    if (codegen.status !== 0) {
      problem(
        `[db:rollback] kysely-codegen failed ` +
    ...
    problem(
      `[db:rollback] An unexpected error occurred: ` +
        `${(e as Error).message}`
    );
    process.exit(1);
  }
